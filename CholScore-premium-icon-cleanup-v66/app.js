  ...
  $("timeline").innerHTML=items.length?items.map(x=>x.kind==="food"
    ?`<div class="log-item food-log-item" data-food-id="${x.id||""}">
        <div class="food-log-main">
          ${x.image?`<img class="food-thumb" src="${esc(x.image)}" alt="${esc(x.name)}" loading="lazy">`:`<div class="food-thumb food-thumb-fallback">${uiLineIcon("food","food-fallback-icon")}</div>`}
...
function renderStaples(){
  ...
  $("staplesRow").innerHTML=staples.map((f,i)=>`
    <button type="button" class="staple-card" data-idx="${i}">
      ${f.image?`<img class="staple-thumb" src="${esc(f.image)}" alt="" loading="lazy">`:`<div class="staple-thumb staple-thumb-fallback">${uiLineIcon("food","food-fallback-icon")}</div>`}
...
function renderFood(){
  ...
  $("foodList").innerHTML=day.foods.length?day.foods.slice().reverse().map(x=>`
    <div class="log-item food-log-item" data-food-id="${x.id||""}">
      <div class="food-log-main">
        ${x.image?`<img class="food-thumb food-thumb-large" src="${esc(x.image)}" alt="${esc(x.name)}" loading="lazy">`:`<div class="food-thumb food-thumb-large food-thumb-fallback">${uiLineIcon("food","food-fallback-icon")}</div>`}
        <div>
  ...
  $("proteinBreakdown").innerHTML=foods.slice().reverse().map(f=>`
    <div class="protein-row">
      <div class="protein-row-main">
        ${f.image?`<img class="protein-thumb" src="${esc(f.image)}" alt="${esc(f.name)}" loading="lazy">`:`<div class="protein-thumb protein-thumb-fallback">${uiLineIcon("food","food-fallback-icon")}</div>`}
        <div>
          <strong>${esc(f.name)}</strong>
...
function setupPremiumExerciseScreen(){
  ...
  style.textContent=`
    ...
    .exercise-training-shell #exerciseList .empty-state:before{content:"◇";display:block;font-size:28px;opacity:.55;margin-bottom:8px;color:#58d9e8}
...
function renderRoutines(){
  ...
  $("routineList").innerHTML=state.routines.map(r=>{
    ...
    return `<div class="routine-card" data-edit-routine="${r.id}">
      ...
      <div class="routine-card-edit-hint">${uiLineIcon("note","note-inline-icon")} Tap the card to edit routine</div>
    </div>`;
  }).join("");
...
function uiLineIcon(name,cls="ui-line-icon"){
  ...
  const paths={
    food:`<circle cx="12" cy="12" r="7.2"/><path d="M3.2 4.2v6.2M5.2 4.2v6.2M4.2 10.4V20M19.2 4.2c-2 1.2-2.6 3.8-1.3 5.7.5.7 1.1 1.1 1.8 1.3V20"/>`,
...
function repTrainingSectionHTML(workouts,dayKey,records){
  ...
  return workouts.map(w=>{
    const rows=(w.exercises||[]).map((ex,i)=>{
      ...
      return `<div class="rep-exercise-row${isPR?" is-pr":""}"><div class="rep-exercise-num">${i+1}</div><div><div class="rep-exercise-name">${esc(ex.name)}${isPR?'<span class="rep-pr-chip">${uiLineIcon("trophy","pr-inline-icon")} PR</span>':""}</div><div class="rep-exercise-meta">${esc(meta)}</div></div><div class="rep-exercise-value">${value}<small>${unit}</small></div></div>`;
    }).join("");
...
function repCardioSectionHTML(cardio,dayKey,records){
  ...
  const rows=cardio.map(a=>{
    ...
    return `<div class="rep-cardio-row${isPR?" is-pr":""}">
      ...
      <div class="rep-cardio-name"><span class="rep-cardio-name-text">${esc(label)}</span>${isPR?'<span class="rep-pr-chip">${uiLineIcon("trophy","pr-inline-icon")} PR</span>':""}</div>
      <div class="rep-cardio-col"><strong>${formatActivityDuration(a.minutes)}</strong></div>
      <div class="rep-cardio-col"><strong>${displayDist>0?`${displayDist} ${unit}`:"—"}</strong>${isDistPR?'<small class="rep-pr-trophy">${uiLineIcon("trophy","pr-inline-icon")}</small>':""}</div>
      <div class="rep-cardio-col"><strong>${pace||"—"}</strong>${isPacePR?'<small class="rep-pr-trophy">${uiLineIcon("trophy","pr-inline-icon")}</small>':""}</div>
    </div>`;
  }).join("");
    ...
    <div class="exercise-row-head">
      ...
      <div class="exercise-row-head-main">
        ...
        <div class="exercise-row-summary">
          ...
          <span class="notes-flag${data.notes?"":" hidden"}">${uiLineIcon("note","note-inline-icon")}</span>
        </div>
      </div>
...
function renderLiveExercises(){
  ...
  $("liveExerciseList").innerHTML=`
    <div class="guided-exercise-card">
      ...
      <button type="button" id="editExerciseNoteBtn" class="exercise-note-btn">${uiLineIcon("note","note-inline-icon")} ${e.notes?"Edit":"Add"} exercise note</button>
    </div>`;
...
function renderPrBadges(elId,badges){
  ...
  el.innerHTML=badges.length?badges.map(b=>`<div class="pr-badge">${uiLineIcon("trophy","pr-inline-icon")} ${b}</div>`).join(""):"";
}
...
(function(){
  if(document.getElementById("cholscoreNoEmojiFallbackV66"))return;
  const s=document.createElement("style");s.id="cholscoreNoEmojiFallbackV66";
  s.textContent=`
    .food-thumb-fallback,.staple-thumb-fallback,.protein-thumb-fallback{
      display:grid!important;place-items:center!important;background:linear-gradient(145deg,rgba(27,45,61,.96),rgba(12,25,38,.98))!important;
      border:1px solid rgba(74,218,225,.24)!important;color:#56dbe3!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.04)!important
    }
    .food-fallback-icon{width:54%!important;height:54%!important;display:block}
    .pr-inline-icon,.note-inline-icon{width:1em;height:1em;display:inline-block;vertical-align:-.14em;flex:0 0 auto}
    .rep-pr-chip,.pr-badge,.notes-flag,.routine-card-edit-hint,.exercise-note-btn{align-items:center}
    .rep-pr-chip,.pr-badge,.routine-card-edit-hint,.exercise-note-btn{gap:6px}
  `;
  document.head.appendChild(s);
})();

CholScore v1.30.2 - Branded native splash screen, replacing Capacitor's default logo
...
## v1.30.2 branded native splash screen, replacing Capacitor's default logo
Flagged directly — a real gap I hadn't addressed, not something deliberately
deferred.

- Checked what Capacitor had actually generated by default before assuming
  anything needed building from scratch: a `Splash.imageset` already
  existed, but it was Capacitor's own "X" logo on a plain white background
  — meaning without this fix, every cold launch would have briefly shown a
  stranger's brand before CholScore's own UI appeared.
- First attempt used the plain `icon-512.png`, but that file has its own
  white background and the wordmark baked directly into the image —
  compositing it onto a dark splash created an ugly white box artifact.
  Caught it by actually looking at the rendered result rather than assuming
  it'd look fine, and found `icon-512-transparent.png` had survived in the
  source folder (even though it had been removed as unused from the
  Capacitor project's own www copy during an earlier cleanup pass) — its
  own background is dark rather than white, so it composites cleanly.
- New splash: the app's dark background colour, the icon centred, a soft
  warm ambient glow behind it echoing the same visual language as the
  in-app redesign, rather than a flat, characterless fill.
- The `LaunchScreen.storyboard` itself was still set to white
  (`systemBackgroundColor`) — fixed that too, since otherwise there'd still
  be a white flash for the split second before the image itself paints.
- Also caught two smaller stale-colour leftovers from the same family of
  issue as the manifest.json fix a few versions back: the inline
  `<style>html,body{background:...}</style>` fallback in index.html's
  `<head>` (there specifically to avoid a flash before the main stylesheet
  loads) was still the pre-redesign hex value. Fixed in both the working
  copy and the actual source file.
- Installed `@capacitor/splash-screen` and set `launchAutoHide: false` —
  the branded splash now stays visible through cold launch until
  `hideNativeSplashScreen()` fires in app.js, right after the app has
  genuinely rendered something (either onboarding or the real app), rather
  than auto-hiding at a fixed moment that could reveal a blank gap while
  the WebView is still starting up.
- Tested what's testable without a physical device: confirmed the new hide
  call safely no-ops with zero errors on the plain web PWA build, same
  verification approach as the local notifications feature.
- `index.html`, `styles.css`, `app.js`, `sw.js`, and all cache-busting query
  strings (including `APP_VERSION`) bumped to `v174`. Capacitor package
  re-synced and re-zipped with the splash screen plugin, the new branded
  splash assets, and the corrected LaunchScreen.storyboard.

## v1.30.1 removed weekly-report notification (contradicted the v1.20.0 on-demand redesign)
Caught immediately after shipping v1.30.0 — a real inconsistency I built
without cross-checking against a much earlier decision.

- The v1.20.0 redesign deliberately removed the Weekly Report's auto-popup
  entirely, replacing it with an always-available Last week / This week /
  Last 4 weeks toggle you check whenever you want. A "your report is ready"
  Monday-morning notification directly contradicts that — there's no
  "ready" moment to announce anymore, since it's always sitting there on
  demand. Removed it rather than try to reframe it into something else.
- Local notifications are down to two, not three: the evening check-out
  reminder and the streak-at-risk warning. Both still work exactly as
  before — re-ran the same scheduling tests to confirm removing the third
  didn't disturb the other two, rather than assumed it was fine.
- `index.html`, `styles.css`, `app.js`, `sw.js`, and all cache-busting query
  strings (including `APP_VERSION`) bumped to `v173`.

## v1.30.0 Capacitor iOS project scaffolded, local notifications wired in
Started once Apple Developer Program approval confirmed. Two real pieces of
progress: an actual working Capacitor iOS project, and local notification
scheduling built into app.js itself.

- Set up a proper Capacitor project (`@capacitor/core`, `@capacitor/cli`,
  `@capacitor/ios`), using the conventional `www/` + native-platform-folders
  layout rather than bolting Capacitor onto the existing flat repo structure.
  `npx cap add ios` genuinely generated a real Xcode project — App.xcodeproj,
  AppDelegate.swift, SceneDelegate.swift, Info.plist — confirming this doesn't
  require Xcode itself to scaffold, only to actually build.
- Found and removed a batch of leftover development scratch files (old
  icon-mask test renders, cropped/zoomed preview images) that had
  accumulated in the working folder but were never referenced anywhere in
  the real app — confirmed via direct reference-checking against
  index.html/app.js/manifest.json before removing anything, not guessed.
- Found a real, worth-flagging discrepancy while checking share-image
  assets: the working copy only has `run-share-template.jpg`, but the app's
  own code specifically constructs `run-share-template.jpeg` (with an "e")
  at runtime. Documented clearly rather than silently guessed at or
  renamed, since it's not possible to tell from here whether this is a
  stale local copy or a genuine gap that's existed on the live site.
- Also caught and fixed a small but real inconsistency: `manifest.json`'s
  `background_color`/`theme_color` were still the pre-redesign hex values
  from before the v1.27.0 visual overhaul, not the current palette. Fixed
  in both the working copy and the actual source repo file — this affects
  native splash/status-bar colour behaviour too, not just the browser PWA
  install prompt.
- **Local notifications, not push** — deliberate choice, not a default.
  Everything in this app has always been client-side with no server and no
  accounts; push would mean standing up a backend purely to decide "should
  Lee get a reminder tonight," which fights the app's whole architecture
  for no benefit local notifications don't already cover. Three scheduled:
  an evening check-out reminder, a streak-at-risk warning, and a "last
  week's report is ready" notification for Monday mornings.
- Rescheduled fresh every time the app opens (cold launch or returning from
  background) rather than set once — a notification scheduled hours ago
  can't know whether you've since checked out, so recomputing the whole set
  each time is what keeps it honest instead of nagging about something
  already done. Fixed IDs per notification "slot" plus cancelling
  everything pending before rescheduling is what guarantees nothing stale
  can ever fire late.
- Tested what's genuinely testable without a physical device: confirmed the
  function safely no-ops with zero errors when running as a plain web
  PWA (the same code ships to both the native app and the live GitHub
  Pages site), then mocked a fake native Capacitor context to verify the
  actual scheduling math — including both edge cases that are easy to get
  wrong: today being Sunday (should schedule the Monday notification for
  tomorrow, not skip a week) and today already being Monday (should
  schedule a full week ahead, not "today" at a time that's likely already
  passed). Both confirmed correct through direct testing, not just review.
  The permission prompt and actual delivery genuinely can't be verified
  without a real device — flagged clearly rather than claimed otherwise.
- Packaged the Capacitor project (config + the real generated `ios/`
  folder) into a handoff zip — deliberately excluding `node_modules`
  (regenerates from `package.json`) and `www/` (should come from the real,
  live GitHub Pages repo, not a snapshot from here that could drift out of
  sync). Included a SETUP.md covering first-build steps, the appId
  decision (permanent once published — needs explicit confirmation before
  building for real), and the run-share-template.jpeg discrepancy above.
- `index.html`, `styles.css`, `app.js`, `sw.js`, `manifest.json`, and all
  cache-busting query strings (including `APP_VERSION`) bumped to `v172`.

## v1.29.0 CholScore+ paywall gating, ready for real purchases later
Built while waiting on Apple Developer Program approval — one app with a
locked/unlocked split, not a separate paid app, since splitting reviews
across two listings and forcing a manual data migration between them (given
everything's stored locally with no account system) would be worse for
users and more to maintain long-term.

- One function — `isPremiumUnlocked()` — is the single thing every gate in
  the app checks. When real purchases exist later (Capacitor + StoreKit),
  only this function's internals need to change to check a real
  entitlement instead of a stored flag; every individual gate stays exactly
  as it is, unchanged.
- Six gates wired in, matching exactly what was designed in the paywall
  mockup: the Rewards tab, Reward Bank, Trends, Reports, the full Day
  Report, and Vacation Mode. Calendar deliberately stays free — matches the
  original free/paid split, not an oversight.
- Genuinely no real purchase flow exists yet — that needs the native wrap.
  Defaults to unlocked for now rather than locked, specifically so this
  doesn't cut off ongoing daily use of Achievements and Rewards while the
  rest of the App Store path is still being built. A manual "simulate
  unlock / simulate locked" toggle in Settings lets both states be tested
  properly in the meantime.
- Proved the migration path directly rather than assumed it: tested a
  simulated existing user's real saved state — one with no `premium` field
  at all, matching exactly what anyone using the app before this feature
  existed actually has stored — and confirmed it correctly resolves to
  unlocked rather than silently locking someone out.
- Tested every gate directly, not just the code path that looks right:
  confirmed the paywall genuinely opens (and the underlying view genuinely
  does *not* activate underneath it) for Rewards, Reward Bank, Trends,
  Reports, and the Day Report while locked; confirmed Calendar stays
  reachable throughout; confirmed the Settings toggle correctly flips
  between both states end to end, not just the flag.
- `index.html`, `styles.css`, `app.js`, `sw.js`, and all cache-busting query
  strings (including `APP_VERSION`) bumped to `v171`.
